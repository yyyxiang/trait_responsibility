...
########## judgments about trait score mutability ##########
# note: all alternative trait scores in the experiment were different from the observed score, 
# so rows where alternative score equals the observed score are marked NA in the csv
mutability_judgment_dat <- read.csv("trait_mutability_pilot.csv", header = T, na.strings = c("", "NA", "null")) %>% 
  filter(total_failed_attention_checks == 0) %>% 
  select(-total_failed_attention_checks) 

mutability_judgment_df <- mutability_judgment_dat %>%
  pivot_longer(matches('^counterfactual_\\d+$'),
               names_to = 'counterfactual', values_to = 'rating') %>%
  mutate(counterfactual = as.integer(sub('counterfactual_', '', counterfactual))) %>%
  filter(!is.na(rating)) %>% 
  group_by(subject, trial) %>%
  mutate(p = rating / sum(rating, na.rm = T)) %>% # normalize ratings within trial, because each slider ranged from 0-100
  ungroup()

inverse_df <- mutability_judgment_df %>% 
  filter(subject == 1) %>% # the trials and predictions are the same for every subject, subsetting to keep one set
  select(trait, observed_score, counterfactual) %>% 
  mutate(p = 1 / abs(counterfactual - observed_score)) %>% 
  group_by(trait, observed_score) %>% 
  mutate(p = p / sum(p))

# Figure S1
pdf('figS1.pdf', onefile = T, width = 10, height = 7)
p <- mutability_judgment_df %>% 
  ggplot(aes(counterfactual, p)) +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0, aes(color = "Data")) +
  stat_summary(fun = 'mean', geom = 'point', size = 2) +
  stat_summary(fun = 'mean', geom = 'line') +
  geom_line(data = inverse_df, linewidth = 1, aes(color = "Inverse function")) +
  facet_grid(trait~observed_score) +
  theme_classic() +
  theme(plot.subtitle = element_text(hjust = 0.5)) +
  scale_color_manual(values = c("Inverse function" = "lightgreen", "Data" = "black"))
p + labs(color = NULL, y = "Mutation probability", x = "Counterfactual score", subtitle = "Observed score")
dev.off()
# note: two participants on two trials responded 0 for all alternative scores, hence the 10 rows of NA removed from plotting
...
# Figure S2
pdf('figS2.pdf', onefile = T, width = 8, height = 5)
...
# Figure S3
pdf("figS3.pdf", onefile = TRUE, width = 16, height = 5)
