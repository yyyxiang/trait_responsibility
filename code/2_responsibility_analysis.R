...
# check if the mean of participants' average responsibility judgments is different from 0
...
########## Model 1: actual contribution ##########
actual_contribution_predictions <- dat_weight
for (tr in all_traits) {
  actual_contribution_predictions[[paste0("responsibility_", tr)]] <- sigmoid(actual_contribution_predictions[[paste0("weight_", tr)]] * actual_contribution_predictions[[tr]])
}

actual_contribution_predictions <- actual_contribution_predictions %>% 
  select(-starts_with("weight")) %>% 
  mutate(model = "actual_contribution")
...
########## combine all models ##########
model_predictions <- rbind(actual_contribution_predictions,
...
cf_models <- setdiff(all_models, "actual_contribution")
...
p0 <- loo_plot(loo_all, model_levels = all_models)
...
# comparing actual contribution and ensemble predictions
a <- dat_long$actual_contribution_fitted
